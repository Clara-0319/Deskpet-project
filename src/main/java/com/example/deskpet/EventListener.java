...
//		System.out.println(x+" "+y);//测试眼睛等部位的位置
    ...
    public void loadImg(int petID,int gifID, double time) {
        ...
        if(gifID!=0) {
            Image newimage;
            if(petID==0)
                newimage = new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/lxh/罗小黑" + gifID + ".gif")));
            else
                newimage = new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/biu/biu" + gifID + ".gif")));
    ...
    public void mainimg(int pet,int key) {
        Image newimage;
        if(pet==0)
            newimage = new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/lxh/罗小黑" + key + ".gif")));
        else
            newimage = new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/biu/biu" + key + ".gif")));
        imageView.setImage(newimage);
